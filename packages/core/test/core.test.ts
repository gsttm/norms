...
describe("norms core", () => {
  ...
  test("normalizes repository paths and line endings", () => {
    expect(normalizeRepositoryPath("/repo", ".norms\\norms\\docs.md")).toBe(".norms/norms/docs.md");
    const markdown = serializeNorm("docs.short", ["**/*.md"], "First.\n\nSecond.").replaceAll("\n", "\r\n");
    const norm = parseNorm(markdown, "docs.md", "repository");
    expect(norm.body).toBe("First.\n\nSecond.");
    expect(generateAgentAdapter([norm])).not.toContain("\r");
    expect(serializeNorm("docs.short", ["**/*.md"], "First.\r\n\r\nSecond.")).not.toContain("\r");
  });
